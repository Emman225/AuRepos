...
/**
 * Fiche de police (CdC § 6.3, § 11) : occupants du séjour, pièce jointe par occupant — jamais le
 * numéro de pièce lui-même (`piece_fournie` reste le seul indicateur, comme au back office).
 * Mêmes textes que la section occupants du back office (`backoffice.reservations.occupants*`).
 */
export function SectionOccupants({ sejourId }: Props) {
  ...
  const occupants = useQuery({
    queryKey: ['agent', 'sejours', sejourId, 'occupants'],
    queryFn: () => listerLesOccupants(sejourId),
  })

  if (!Array.isArray(occupants.data) || occupants.data.length === 0) return null

  const invalider = () => queryClient.invalidateQueries({ queryKey: ['agent', 'sejours', sejourId, 'occupants'] })
