...
interface Props {
  ...
  onFermer: () => void
  onCheckOut: (sejour: ReservationBackOffice) => void
}

/** Check-out (CdC § 6.1) : consommations facturées, décision manuelle sur la caution, facture unique du séjour. */
export function FormulaireCheckOut({ ouvert, sejour, onFermer, onCheckOut }: Props) {
  ...
  const consommations = useQuery({
    queryKey: ['backoffice', 'sejours', sejour?.id, 'consommations'],
  ...
  return (
    ...
    >
      ...
      <Form layout="vertical" style={{ marginTop: 16 }}>
        <Form.Item label={t('backoffice.reservations.checkOut.cautionRetenue')} htmlFor="champ-caution-retenue">
          <InputNumber
            id="champ-caution-retenue"
            style={{ width: '100%' }}
            size="large"
            min={0}
            step={1000}
