...
/** État des lieux d'entrée / de sortie (CdC § 6.1) : inventaire, photos, compteurs, signature écran, comparaison, PDF. */
export function SectionEtatsDesLieux({ sejourId }: Props) {
  const { t } = useTranslation()
  const queryClient = useQueryClient()

  const etats = useQuery({
    queryKey: ['backoffice', 'sejours', sejourId, 'etats-des-lieux'],
    queryFn: () => listerLesEtatsDesLieux(sejourId),
  })

  const invalider = () => queryClient.invalidateQueries({ queryKey: ['backoffice', 'sejours', sejourId, 'etats-des-lieux'] })
